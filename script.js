...
function renderCategories(snapshot){
  ...
  snapshot.forEach(child=>{
    ...
    const imgSrc = catData.img || 'assets/default-category.png';
    el.innerHTML = `
      <div class="cat-image"><img data-src="${imgSrc}" alt="${name}" loading="lazy"/></div>
      <div class="cat-name">${name}</div>
      <div class="cat-note">Sizin için seçtiklerimiz</div>
      <div class="cat-chip">0</div>
    `;

    const img = el.querySelector('img');
    observer.observe(img);

    // Ürün sayısını çek ve güncelle
    db.ref('products/'+name).once('value').then(prodSnap=>{
      const count = prodSnap.exists() ? prodSnap.numChildren() : 0;
      const chip = el.querySelector('.cat-chip');
...
function renderProducts(){
  ...
  items.forEach(p=>{
    ...
    card.innerHTML=`
      <div class="cover">
        ${p.img?`<img data-src="${p.img}" alt="${p.name}" loading="lazy"/>`:''}
